...
var paddirmove = 0, paddirinitted = false;
...
function Game (can, ctx) {
	...
	/* Info box on items */
	setTimeout(function () {
		// this.info_box.activated = true;
		// this.info_box.trigger(true, {id: 2}, 3)
		// console.log(this.info_box)
	}.bind(this), 2000)
	this.info_box = {
	...
	this.update_craft_buttons = function () {
		...
		for (var i = 0 ; i < craft.length ; i++) {
			...
			craft[i].info.translate.x = pmog.translate.x + (col * (65 * pmog.scale));
			craft[i].info.translate.y = pmog.translate.y - (row * (70 * pmog.scale));
			craft[i].info.scale = pmog.scale;
			craft[i].info.width = pmog.size.x;
			craft[i].info.height = pmog.size.y;
			col += 1;
			if (i > 0 && !(i % 9)) {
				col = 0;
				row += 1;
			...
			// x += ((10 * pmog.scale) + craft[i].info.width);
		}
	}
	...
	this.run = function () {
		...
		Tub.updateLeaderBagBookVisibility(true);
		_this.initPadDir();
	}
	...
	this.draw = function () {
		...
		if (shouldShowPadSemi) this.drawPad();
	}

	this.drawPad = function () {
		// console.log('drawing dpad')
		// console.log(IMAGES.PAD_SEMI.width, IMAGES.PAD_SEMI.height)

		ctxDrawImage(ctx, IMAGES.PAD_SEMI, panStartX - 59, panStartY - 55, 118, 110);
	}

	this.initPadDir = function () {
		if (paddirinitted) return;
		paddirinitted = true;
		var left = document.querySelector('#pad-left'), right = document.querySelector('#pad-right'),
			top = document.querySelector('#pad-top'), bottom = document.querySelector('#pad-bottom');
		left.addEventListener('touchstart', this.handlePadDirDown.bind(this, 1, 3.12));
		left.addEventListener('touchend', this.handlePadDirUp.bind(this));
		right.addEventListener('touchstart', this.handlePadDirDown.bind(this, 2, -0.02));
		right.addEventListener('touchend', this.handlePadDirUp.bind(this));
		top.addEventListener('touchstart', this.handlePadDirDown.bind(this, 8, -1.58));
		top.addEventListener('touchend', this.handlePadDirUp.bind(this));
		bottom.addEventListener('touchstart', this.handlePadDirDown.bind(this, 4, 1.58));
		bottom.addEventListener('touchend', this.handlePadDirUp.bind(this));
	}

	this.drawPadDir = function () {

	}

	this.handlePadDirDown = function (hmrmove, hmrangl, evt) {
		hammermove = hmrmove;
		hammerangle = hmrangl;
		evt.target.style.opacity = 1;
	}

	this.handlePadDirUp = function (evt) {
		hammermove = 0;
  ...
  this.current_cursor = false;
	this.trigger_mousemove = function (evt) {
		...
		//
		if (cursor && gameTouchEndCallback) gameTouchEndCallback(null, true);
	}
