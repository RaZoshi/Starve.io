...
function ctxDrawImage (ctx, img, b, c, d, e, f, g, h, i, mog) {
	...
	if (isMobile && mog) {
		...
		if (img.tryLoad === undefined || img.tryLoad () === 1) {
			...
			if (i !== undefined)
				...
				drawMobileElement(ctx, img, mog.translate, mog.size, mog);
			}
...
function draw_info_box () {
	...
	var inv = box.id;
	var img, a, mog = Mogs.InfoBoxInCanvas;
	...
	if (box.activated === 1 && box.display === 1) {
		...
		if (box.craft === 1)
			ctxDrawImage (ctx, INFO_BOX[inv], box.x, box.y, 333, 333, a, a, a, a, mog);
		else {
			if (box.x - 80 + INFO_BOX[inv].width > canw)
				ctxDrawImage (ctx, INFO_BOX[inv], canw - INFO_BOX[inv].width - 10, box.y - INFO_BOX[inv].height - 10, 333, 333, a, a, a, a, mog);
			else
				ctxDrawImage (ctx, INFO_BOX[inv], box.x - 80, box.y - INFO_BOX[inv].height - 10, 333, 333, a, a, a, a, mog);
		}
	}
}
...
function draw_slot_number (n, button, push) {
	...
	if (button.info.state == BUTTON_CLICK || push)
		y += scale * 5;
	var w = img.width * button.info.scale;
	var h = img.height * button.info.scale;
	ctxDrawImage (ctx, img, x, y, w, h);
}
...
function draw_amount (n, button) {
	...
	var w = img.width * button.info.scale;
	var h = img.height * button.info.scale;
	var x = button.info.translate.x + button.info.width - (w);
	var y = button.info.translate.y + button.info.height - (h);
	if (button.info.state == BUTTON_CLICK)
		y += scale * 5;
	ctxDrawImage (ctx, img, x, y, w, h);
	// console.log(button.info.scale)
}
...
function draw_bigmap () {
	...
	var scale = Math.min(canw, canh) / 800, mapSize = 600 * scale;
	ctx.translate(canw2 - (mapSize * .5), canh2 - (mapSize * .5));
	...
	if (game.minimap.marker.x > -1) {
		...
		circle (ctx, x + game.minimap.marker.x * 3,
		        y + game.minimap.marker.y * 3, 14);
		ctx.fill ();
	}
	...
	for (var i = 0 ; i < players.length ; i++) {
		...
		circle (ctx, x + 600 * (p.x / (world.nw * 100)),
			y + 600 * (p.y / (world.nh * 100)), 6);
		ctx.fill ();
	}
	...
	if (u) {
		...
		circle (ctx, x + 600 * (u.x / (world.nw * 100)),
		        y + 600 * (u.y / (world.nh * 100)), 12);
		ctx.fill ();
	}
...
function draw_status () {
	...
	var pmog = Mogs.BaggBoardElement;
	var mog = Mogs.AutoFeedIcon;
	mog.scale = pmog.scale * 1.3;
	mog.translate.x = pmog.translate.x;
	mog.translate.y = pmog.translate.y + (110 * mog.scale);
	user.auto_feed.translate.x = mog.translate.x;
	user.auto_feed.translate.y = mog.translate.y;
	...
	if (user.auto_feed.enabled) {
		...
		ctxDrawImage (ctx, img, user.auto_feed.translate.x, user.auto_feed.translate.y + y,
			img.width * mog.scale, img.height * mog.scale);
		y +=  70 * mog.scale;
	}
	...
	if (user.bandage > 0) {
		...
		if (user.bandage < 9)
			ctxDrawImage (ctx, bandage, mog.translate.x, mog.translate.y, bandage.width * mog.scale, bandage.height * mog.scale);
		else
			ctxDrawImage (ctx, bandage, mog.translate.x, mog.translate.y, bandage.width * mog.scale, bandage.height * mog.scale);
	}
}
...
function draw_leaderboard () {
	...
	//
	// 			ctx2.drawImage (sprite[SPRITE.YOUR_SCORE], 15 * scale, (96 + i * 22) * scale);
	// 			ctx2.drawImage (create_text (scale, Utils.simplify_number (world.players[user.id].score),
	// 					15 * scale, "#FFF"), 100 * scale, (96 + i * 22) * scale);
	// 		}
	// 	}
	// }
	//
...
function draw_clock () {
	...
	mog.translate.x = mog.tx * (canw / mog.w);
	mog.translate.y = mog.ty * (canh / mog.h);
	mog.scale = finalScale / mog.w;

	ctx.scale (mog.scale, mog.scale);
	c.translate.x = mog.translate.x;
	c.translate.y = mog.translate.y;
...
function draw_ui_crafting () {
	...
	if (!craft.crafting && craft.preview > 0) {
		...
		if (INV_INFOS[craft.preview].grid === 0) {
			// temporarily disabled PRESS G TO CHANGE
	...
	if (craft.crafting) {
		...
		for (var i = 0 ; i < craft.can_craft.length ; i++) {
			...
			if (craft.id == button.id) {
				...
				var h = button.info.height;
				var h2 = 5 * button.info.scale;
				round_rect (ctx, button.info.translate.x,
				            button.info.translate.y + h * (1 - craft.timeout.v) - h2,
					    button.info.width,
					    h * craft.timeout.v,
					    scale * 10);
...
function draw_ui_inventory () {
	...
	mog.size.y = mog.size.x / sideRatio;
	mog.tx = 30;
	mog.ty = canh - (mog.size.y * 1.2);
	// mog.translate.x = mog.tx * (canw / mog.w);
	// mog.translate.y = mog.ty * (canh / mog.h);
	mog.translate.x = mog.tx * (finalScale / mog.w);
	mog.translate.y = mog.ty;
	mog.scale = finalScale / mog.w;

	// console.log(mog.translate.x, mog.translate.y)
	...
	for (var i = 0 ; i < inv.can_select.length ; i++) {
		...
		var push = false;
		// button.info.translate.x = 111;/
		// button.info.translate.y = 111;
		// button.info.width = 33;
		// button.info.height = 66;


		// console.log(mog.size.x, mog.size.y, mog.translate.x, mog.translate.y)
		button.info.width = mog.size.x;
		button.info.height = mog.size.y;
		button.info.translate.x = mog.translate.x + ((i * mog.size.x * 1.1));
		button.info.translate.y = mog.translate.y;
		button.info.scale = mog.scale;
	...
	if (button && i < inv.max) {
		...
		x = button.info.translate.x + 3;
		y = button.info.translate.y - 3;
		w = mog.size.x;
		h = mog.size.y;
		for (var j = 1 ; i < inv.max ; i++, j++)
			ctxDrawImage (ctx, img, x + j * (w + 5), y, w, h);
	}
}
...
function draw_gauges () {
	...
	ctx.scale (mog.scale, mog.scale);
	this.translate.x = mog.translate.x;
	this.translate.y = mog.translate.y;
	...
	ctx.fillRect (this.translate.x + 37 * scale, this.translate.y + 10 * scale, user.gauges.life.x * 115 * scale, 30 * scale);
	ctx.globalAlpha = 1;

	if (user.gauges.hunger.x < 0.35) {
		ctx.fillStyle = "#8F050A";
		ctx.globalAlpha = user.gauges.warn_hunger.v;
		ctx.fillRect (this.translate.x + 190 * scale, this.translate.y + 10 * scale, 115 * scale, 30 * scale);
		ctx.globalAlpha = 1;
	}
	ctx.fillStyle = "#AF352A";
	ctx.fillRect (this.translate.x + 190 * scale, this.translate.y + 10 * scale, user.gauges.hunger.x * 115 * scale, 30 * scale);

	if (user.gauges.cold.x < 0.35) {
		ctx.fillStyle = "#366B91";
		ctx.globalAlpha = user.gauges.warn_cold.v;
		ctx.fillRect (this.translate.x + 340 * scale, this.translate.y + 10 * scale, 115 * scale, 30 * scale);
		ctx.globalAlpha = 1;
	}
	ctx.fillStyle = "#669BB1";
	ctx.fillRect (this.translate.x + 340 * scale, this.translate.y + 10 * scale, user.gauges.cold.x * 115 * scale, 30 * scale);

	if (user.gauges.cold.x > 0.99) {
		if (user.gauges.warm.x < 0.35) {
			ctx.fillStyle = "#6C4036";
			ctx.globalAlpha = user.gauges.warn_warm.v;
			ctx.fillRect (this.translate.x + 490 * scale, this.translate.y + 10 * scale, 115 * scale, 30 * scale);
			ctx.globalAlpha = 1;
		}
		ctx.fillStyle = "#9C4036";
		ctx.fillRect (this.translate.x + 490 * scale, this.translate.y + 10 * scale, (1 - user.gauges.warm.x) * 115 * scale, 30 * scale);
	}
	...
	ctx.fillRect (this.translate.x + 490 * scale, this.translate.y + 10 * scale, user.gauges.thirst.x * 115 * scale, 30 * scale);
	...
	if (user.gauges.oxygen.x < 0.99) {
		...
		var x = this.translate.x + 20;
		var y = this.translate.y + 60;

		ctx.fillStyle = "#BBE8EF";
		ctx.fillRect (x + 5, y + 5 * scale, 570 * scale * user.gauges.oxygen.x, 10 * scale);

		if (user.gauges.oxygen.x < 0.35) {
			ctx.fillStyle = "#9BA8AF";
			ctx.globalAlpha = user.gauges.warn_oxygen.v;
			ctx.fillRect (x + 5, y + 5, 570 * scale, 10 * scale);
			ctx.globalAlpha = 1;
		}

		ctxDrawImage (ctx, sprite[SPRITE.OXYGEN], x, y, 580 * scale, 20 * scale);
...
function draw_world () {
	...
	if (ui.quality) {
		...
		// HALLOWEEN UPDATE
		/*
		draw_map_transition (draw_map_objects, is, ie, js, je, SPRITE.TREE_WEB1,   "t", 5, 0, 1);
		draw_map_transition (draw_map_objects, is, ie, js, je, SPRITE.TREE_WEB2,   "t", 5, 0, 2);
		draw_map_transition (draw_map_objects, is, ie, js, je,  SPRITE.TREE_BRANCH_WEB1, "b", 3, 0, 1);
		draw_map_transition (draw_map_objects, is, ie, js, je,  SPRITE.TREE_BRANCH_WEB2, "b", 3, 0, 2);
		*/
	}

	//draw_map_transition (draw_map_objects, is, ie, js, je, SPRITE.ISLAND_BLOCK, "wtb", 0);
	...
	/* Draw tomato seed background */
	var seed = world.units[ITEMS.TOMATO_SEED];
	for (var i = 0 ; i < seed.length ; i++)
		draw_bg_transition (seed[i]);

	/* Draw seed firstground */
	var seed = world.units[ITEMS.SEED];
	for (var i = 0 ; i < seed.length ; i++)
	...
	*/
	...
	for (var i = 0 ; i < players.length ; i++) {
		...
		var p = players[i];
		if (players[i].vehicle !== INV.BABY_DRAGON && players[i].vehicle !== INV.BABY_LAVA &&
		    players[i].vehicle !== INV.HAWK && players[i].vehicle !== INV.PLANE &&
		    players[i].vehicle !== INV.NIMBUS) {
			...
			if (p.tower === 0) {
				...
				if (p.tower_fx > 0.001) {
					...
					ctx.save ();
					ctx.scale (spd, spd);
					user.cam.x /= spd; user.cam.y /= spd; p.x /= spd; p.y /= spd; p.r.x /= spd; p.r.y /= spd;
					p.draw_vehicle ();
					p.draw ();
					user.cam.x *= spd; user.cam.y *= spd; p.x *= spd; p.y *= spd; p.r.x *= spd; p.r.y *= spd;
					ctx.restore ();
	...
	/* Draw unbreakable map object (next) */
	...
	// WINTER UPDATE
	/*draw_map_transition (draw_map_objects, is, ie, js, je, SPRITE.TREE_SNOW1,   "t", 5, 0, 1);
	draw_map_transition (draw_map_objects, is, ie, js, je, SPRITE.TREE_SNOW2,   "t", 5, 0, 2);
	draw_map_transition (draw_map_objects, is, ie, js, je,  SPRITE.TREE_BRANCH_SNOW1, "b", 3, 0, 1);
	draw_map_transition (draw_map_objects, is, ie, js, je,  SPRITE.TREE_BRANCH_SNOW2, "b", 3, 0, 2);*/
	...
	/* Draw player on tower */
	var players = world.units[ITEMS.PLAYERS];
	for (var i = 0 ; i < players.length ; i++) {

		var p = players[i];
		if (p.tower === 1 && (p.speed < 180 ||
			(p.vehicle !== INV.BABY_DRAGON && p.vehicle !== INV.BABY_LAVA && p.vehicle !== INV.NIMBUS &&
		         p.vehicle !== INV.HAWK && p.vehicle !== INV.PLANE))) {
	...
	for (var i = 0 ; i < players.length ; i++) {
		...
		if ((p.vehicle === INV.BABY_DRAGON || p.vehicle === INV.BABY_LAVA || p.vehicle === INV.NIMBUS ||
		     p.vehicle === INV.HAWK || p.vehicle === INV.PLANE) && p.speed > 180) {
